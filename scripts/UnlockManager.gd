...
func can_unlock_bundle(bundle_id: String) -> bool:
	if is_bundle_unlocked(bundle_id) or not bundles.has(bundle_id):
		return false
...
func unlock_bundle(bundle_id: String, emit_signals: bool = true) -> Array:
	if is_bundle_unlocked(bundle_id) or not bundles.has(bundle_id):
		return []
	...
	if emit_signals:
		...
		bundle_unlocked.emit(bundle_id, cards)
	
	unlocked_bundles.append(bundle_id)
	bundle_progress[bundle_id] = bundle.requirement_value
	return cards

func track_progress(progress_type: String, value: int, extra_data: Dictionary = {}):
	var progress_made = false
	var bundles_to_unlock: Array[String] = []
	...
	for bundle_id in bundles.keys():
		...
		var new_progress = max(0, _calculate_progress(bundle, progress_type, value, extra_data, old_progress))
		...
		if new_progress >= bundle.requirement_value:
	...
	match bundle.requirement_type:
		...
		"hybrid_cards_played":
			if progress_type == "card_played" and extra_data.get("card_type", "") == "hybrid":
				return min(old_progress + value, bundle.requirement_value)
		"high_hp_victories":
			if progress_type == "game_won" and extra_data.get("final_hp", 0) >= 15:
				return min(old_progress + 1, bundle.requirement_value)
		"games_completed":
			if progress_type == "game_ended":
				return min(old_progress + 1, bundle.requirement_value)
		"low_hp_recoveries":
			if progress_type == "game_won" and extra_data.get("was_at_low_hp", false):
				return min(old_progress + 1, bundle.requirement_value)
		"total_wins":
...
func get_progress_text(bundle_id: String) -> String:
	...
	var required = max(1, bundle.requirement_value)
	...
	match bundle.requirement_type:
		...
		"survive_turns":
			return str(displayed_current) + "/" + str(required) + " turns survived" if displayed_current < required else "COMPLETED (" + str(required) + "+ turns)"
		"speed_win_hard":
			return "0/1 speed wins" if displayed_current < required else "COMPLETED"
		"hybrid_cards_played":
...
func load_progress():
	...
	if typeof(data) != TYPE_DICTIONARY:
		push_error("Invalid unlock save data format")
		reset_all_progress()
		return
