...
func add_template(template: Dictionary, weight_override: int = -1):
	...
	if not _is_card_available(card_name):
		return
...
func generate_deck(deck_size: int, config: DeckConfig = null) -> Array:
	...
	if config and config.guaranteed_rarities:
		_add_guaranteed_cards(deck, rarity_counts)
...
func validate_pool() -> Dictionary:
	...
	for entry in _card_pool:
		...
		if _is_card_available(card_name):
			available_count += 1
		else:
			locked_count += 1
...
func _is_card_available(card_name: String) -> bool:
...
func _add_guaranteed_cards(deck: Array, rarity_counts: Dictionary):
